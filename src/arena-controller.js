...
export class ArenaController extends DurableObject {
  ...
  async getUatDiagnostics(){const state=await this.ctx.storage.get(STATE_KEY),counts={};for(const table of ["campaign_archives","opportunity_scans","opportunity_records","agent_decisions","decision_outcomes","round_scoring_settlements"]){try{counts[table]=this.ctx.storage.sql.exec(`SELECT COUNT(*) AS count FROM ${table}`).one().count;}catch(error){counts[table]={error:error instanceof Error?error.message:"Unavailable"};}}return {ok:true,campaignId:state?.campaign?.id||null,strategyVersions:STRATEGY_VERSIONS,auditCounts:counts,lastOpportunityError:state?.opportunity?.lastError||null,activeScanCycleId:state?.opportunity?.activeBoard?.scanCycleId||null,candidateCount:state?.candidateUniverse?.activeSnapshot?.candidates?.length||0,activities:Object.fromEntries(ARENA_CONFIG.agents.map(id=>[id,{status:state?.agents?.[id]?.activity?.status||null,message:state?.agents?.[id]?.activity?.message||null,decision:state?.agents?.[id]?.activity?.decision||null,updatedAt:state?.agents?.[id]?.activity?.updatedAt||null}]))};}
