...
import { AGENT_REGISTRY, STRATEGY_METADATA, STRATEGY_VERSIONS, validateAgentDecision } from "./strategies.js";
import { finalizeResolvableBoard, isExactCoinbaseUsdProduct } from "./opportunity-contract.js";
...
export class ArenaController extends DurableObject {
  ...
  async getUatDiagnostics(){const state=await this.ctx.storage.get(STATE_KEY),counts={};for(const table of ["campaign_archives","opportunity_scans","opportunity_records","agent_decisions","decision_outcomes","round_scoring_settlements"]){try{counts[table]=this.ctx.storage.sql.exec(`SELECT COUNT(*) AS count FROM ${table}`).one().count;}catch(error){counts[table]={error:error instanceof Error?error.message:"Unavailable"};}}const rows=this.ctx.storage.sql.exec("SELECT decision_id,strategy_version,scan_cycle_id,selected_opportunity_id,decision,allocation,reason_code,reason_detail,decided_at,execution_status,order_id FROM agent_decisions WHERE agent_id = ? ORDER BY decided_at ASC","ATLAS").toArray(),view=row=>{let detail={};try{detail=JSON.parse(row.reason_detail||"{}");}catch{}return {decisionId:row.decision_id,strategyVersion:row.strategy_version,scanCycleId:row.scan_cycle_id,selectedOpportunityId:row.selected_opportunity_id,decision:row.decision,allocation:row.allocation?JSON.parse(row.allocation):null,reasonCode:row.reason_code,reasonSummary:detail.reasonSummary||null,candidateSource:detail.candidateSource||null,candidateId:detail.candidateId||null,positionCount:detail.positionCount,availableSlots:detail.availableSlots,decidedAt:row.decided_at,executionStatus:row.execution_status,orderId:row.order_id};};return {ok:true,campaignId:state?.campaign?.id||null,strategyVersions:STRATEGY_VERSIONS,auditCounts:counts,lastOpportunityError:state?.opportunity?.lastError||null,activeScanCycleId:state?.opportunity?.activeBoard?.scanCycleId||null,candidateCount:state?.candidateUniverse?.activeSnapshot?.candidates?.length||0,firstPrestonDecision:rows.length?view(rows[0]):null,latestPrestonDecision:rows.length?view(rows[rows.length-1]):null,activities:Object.fromEntries(ARENA_CONFIG.agents.map(id=>[id,{status:state?.agents?.[id]?.activity?.status||null,message:state?.agents?.[id]?.activity?.message||null,decision:state?.agents?.[id]?.activity?.decision||null,updatedAt:state?.agents?.[id]?.activity?.updatedAt||null}]))};}
  ...
  async alarm() {
    ...
    try {
      ...
      let accepted,assets,movers;
      try{const candidate=await provider.getOpportunityBoard();movers=await provider.getMovers();assets=await provider.getCandidateMarketContext(candidate.board,movers,heldProducts);accepted=await this.acceptOpportunityBoard({...candidate,...await finalizeResolvableBoard(candidate.board,assets,candidate.rejectionCodes)},now);}catch(error){const retained=state.opportunity?.activeBoard;if(!retained)throw error;accepted={board:retained,rejectionCodes:[]};movers={updatedAt:null,stale:true,assets:[]};assets=await provider.getCandidateMarketContext(retained,movers,heldProducts);}
      state=await this.loadAndReconcile();
      state.candidateUniverse={activeSnapshot:buildCandidateSnapshot(accepted.board,movers,assets,now),lastAcceptedAt:iso(now)};
      state=await this.updateMarketAndActivities(state,assets,accepted.board,now);
      for(const agentId of ARENA_CONFIG.agents)await this.runAgentDecision(agentId,assets,accepted.board,state.candidateUniverse.activeSnapshot,now);
    } catch(error) {
  ...
  async executeOrderWithQuote(input,quote,requestId) {
    ...
    return this.ctx.storage.transaction(async txn => {
      ...
      if(input.side==="BUY"){
        validatePortfolioEntry(account,input.productId);
        const candidate=findEligibleCandidate(state.candidateUniverse?.activeSnapshot,input.candidateId,input.productId,now);
        if(!candidate)throw new ArenaError("CANDIDATE_NOT_ACTIVE","BUY requires a fresh, eligible candidate from the active snapshot.",409);
        input={...input,opportunityId:candidate.opportunityId,scanCycleId:candidate.scanId,boardHash:state.opportunity?.activeBoard?.boardHash||null,candidateSource:candidate.source,candidateId:candidate.candidateId,moverId:candidate.moverId,sourceObservedAt:candidate.observedAt,assetLogoUrl:candidate.logoUrl||null};
      }
  ...
  async runAgentDecision(agentId,assets,board,candidateSnapshot,now){
    ...
    const candidates=candidateSnapshot?.candidates||[],evaluated=candidates.map(item=>item.candidateId),positionCount=Object.keys(state.agents[agentId].positions).length,availableSlots=Math.max(0,ARENA_CONFIG.maximumOpenPositions-positionCount);
    const decision=validateAgentDecision(AGENT_REGISTRY[agentId].decide({agent:state.agents[agentId],candidates,assets,round:state.round,campaign:state.campaign,availableSlots,cash:state.agents[agentId].cashUsd,equity:state.agents[agentId].accountEquityUsd,costEstimates:executionModel(this.env),now}));
    const strategyMetadata=STRATEGY_METADATA[agentId],decisionId=`decision-${crypto.randomUUID()}`,record={decisionId,campaignId:state.campaign.id,roundNumber:state.round.number,agentId,strategyVersion:STRATEGY_VERSIONS[agentId],scanCycleId:board.scanCycleId,boardHash:board.boardHash,evaluatedOpportunityIds:evaluated,selectedOpportunityId:decision.selectedCandidateId,decision:decision.decision,allocation:decision.allocation,agentConfidence:decision.confidence,reasonCode:decision.reasonCode,reasonDetail:JSON.stringify({candidateSource:decision.candidateSource,candidateId:decision.selectedCandidateId,positionCount,availableSlots,cash:state.agents[agentId].cashUsd,equity:state.agents[agentId].accountEquityUsd,reasonCodes:decision.reasonCodes||[decision.reasonCode],reasonSummary:decision.reasonSummary||null,dialogueKey:decision.dialogueKey||null,idempotencyKey:decision.idempotencyKey||null,supportingMetrics:decision.supportingMetrics||null,riskMetrics:decision.riskMetrics||null,strategyMetadata}),decidedAt:iso(now),executionStatus:"NOT_REQUESTED",orderId:null};
    this.storeDecision(record);
    activity.decisionSequenceId=sequenceId;activity.decision={decision:decision.decision,productId:decision.productId,reasonCode:decision.reasonCode,dialogueKey:decision.dialogueKey||null};activity.confidence=decision.confidence;activity.allocationPercent=decision.allocation?.value??null;activity.selectedProductId=decision.productId;activity.updatedAt=iso(now);activity.message=decision.dialogueText||decision.reasonCode;
    recordDecisionScoring(state,agentId,{...decision,selectedOpportunityId:decision.selectedCandidateId},{...board,opportunities:candidates.map(c=>({...c,opportunityId:c.candidateId,qualified:c.eligible,tradability:c.eligible?"TRADABLE":"UNRESOLVED"}))},now);
  ...
  async buildArenaPayload(state, mark) {
    ...
    return { ok: true, service: SERVICE, version: VERSION, simulation: true, serverTime: new Date().toISOString(), campaign: campaignView(state.campaign), round: decorateRound(state.round), scoreboard: scoreboardPayload(state), scoring: publicScoring(state), halftime: publicHalftime(state), agents: publicAgents(state.agents), recentTrades: state.trades.slice(-ARENA_CONFIG.recentTradeLimit).reverse().map(publicTrade), market: state.market, candidateUniverse:state.candidateUniverse?.activeSnapshot||null, portfolioLimit:ARENA_CONFIG.maximumOpenPositions, executionModel: executionModel(this.env) };
  }
}
...
export function executeBuy(state, account, input, quote, model, orderId, now) {
  ...
  const metadata=input.strategyMetadata||{},provenance={candidateSource:input.candidateSource,candidateId:input.candidateId,moverId:input.moverId||null,sourceObservedAt:input.sourceObservedAt,assetLogoUrl:input.assetLogoUrl||null,opportunityId:input.opportunityId,scanCycleId:input.scanCycleId,boardHash:input.boardHash,decisionId:input.decisionId,strategyId:metadata.strategyId||null,strategyName:metadata.strategyName||null,strategyAuthor:metadata.strategyAuthor||null,strategyVersion:input.strategyVersion,arenaRulesVersion:metadata.arenaRulesVersion||"1.1",strategyIntegrationVersion:metadata.integrationVersion||null,strategyCommit:metadata.commit||null,executionModelVersion:model.version};
...
export function executeSell(state, account, input, quote, model, orderId, now) {
  ...
  const provenance={candidateSource:position.candidateSource,candidateId:position.candidateId,moverId:position.moverId||null,sourceObservedAt:position.sourceObservedAt,assetLogoUrl:position.assetLogoUrl||null,opportunityId:position.opportunityId,scanCycleId:position.scanCycleId,boardHash:position.boardHash,decisionId:position.decisionId,closeDecisionId:input.decisionId||null,strategyId:position.strategyId||null,strategyName:position.strategyName||null,strategyAuthor:position.strategyAuthor||null,strategyVersion:position.strategyVersion,arenaRulesVersion:position.arenaRulesVersion||"1.1",strategyIntegrationVersion:position.strategyIntegrationVersion||null,strategyCommit:position.strategyCommit||null,executionModelVersion:model.version};
