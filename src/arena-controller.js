  ...
  async executeOrderWithQuote(input,quote,requestId) {
    ...
    return this.ctx.storage.transaction(async txn => {
      ...
      if(input.side==="BUY"){
        ...
        input={...input,opportunityId:candidate.opportunityId,scanCycleId:candidate.scanId,boardHash:state.opportunity?.activeBoard?.boardHash||null,candidateSource:candidate.source,candidateId:candidate.candidateId,moverId:candidate.moverId,sourceObservedAt:candidate.observedAt};
      }
...
export function executeSell(state, account, input, quote, model, orderId, now) {
  ...
  const provenance={candidateSource:position.candidateSource,candidateId:position.candidateId,moverId:position.moverId||null,sourceObservedAt:position.sourceObservedAt,opportunityId:position.opportunityId,scanCycleId:position.scanCycleId,boardHash:position.boardHash,decisionId:position.decisionId,closeDecisionId:input.decisionId||null,strategyId:position.strategyId||null,strategyName:position.strategyName||null,strategyAuthor:position.strategyAuthor||null,strategyVersion:position.strategyVersion,arenaRulesVersion:position.arenaRulesVersion||"1.1",strategyIntegrationVersion:position.strategyIntegrationVersion||null,strategyCommit:position.strategyCommit||null,executionModelVersion:model.version};
