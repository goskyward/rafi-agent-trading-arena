...
const providerEnv={OPPORTUNITY_ENGINE_BASE_URL:"  https://rafi-crypto-movers.camronra2020.workers.dev////  "},provider=new OpportunityEngineMarketProvider(providerEnv);assert.equal(provider.env,providerEnv);assert.equal(provider.baseUrl,"https://rafi-crypto-movers.camronra2020.workers.dev");assert.equal(provider.resolveUrl("/health").href,"https://rafi-crypto-movers.camronra2020.workers.dev/health");assert.equal(provider.resolveUrl("/movers?view=all").href,"https://rafi-crypto-movers.camronra2020.workers.dev/movers?view=all");const savedFetch=globalThis.fetch;globalThis.fetch=async input=>{assert.equal(input,"https://rafi-crypto-movers.camronra2020.workers.dev/health");return Response.json({ok:true});};const providerHealth=await provider.health();globalThis.fetch=savedFetch;assert.equal(providerHealth.reachable,true);assert.equal(providerHealth.status,"online");
