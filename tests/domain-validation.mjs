...
const providerEnv={OPPORTUNITY_ENGINE_BASE_URL:"  https://rafi-crypto-movers.camronra2020.workers.dev////  ",ENABLE_DEV_DIAGNOSTICS:"true"},provider=new OpportunityEngineMarketProvider(providerEnv);assert.equal(provider.env,providerEnv);assert.equal(provider.baseUrl,"https://rafi-crypto-movers.camronra2020.workers.dev");assert.equal(provider.resolveUrl("/health").href,"https://rafi-crypto-movers.camronra2020.workers.dev/health");assert.equal(provider.resolveUrl("/movers?view=all").href,"https://rafi-crypto-movers.camronra2020.workers.dev/movers?view=all");const savedFetch=globalThis.fetch;globalThis.fetch=async input=>{assert.equal(input.href,"https://rafi-crypto-movers.camronra2020.workers.dev/health");return Response.json({ok:true,service:"RA-FI Opportunity Engine",version:"test"},{headers:{"content-type":"application/json"}});};const diagnosticHealth=await provider.health(),protectedDiagnostic=await provider.diagnosticHealth();globalThis.fetch=savedFetch;assert.equal(diagnosticHealth.upstreamHealthUrl,"https://rafi-crypto-movers.camronra2020.workers.dev/health");assert.equal(diagnosticHealth.httpStatus,200);assert.equal(diagnosticHealth.reachable,true);assert.deepEqual(protectedDiagnostic,{status:200,service:"RA-FI Opportunity Engine",version:"test",contentType:"application/json"});
