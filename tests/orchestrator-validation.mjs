...
assert.match(source, /ensureEvaluationAlarm\(latest,"alarm-cycle"\)/, "active campaigns must reschedule through the idempotent alarm guard");
...
assert.match(source, /async alarm\(\)[\s\S]*ensureActiveCampaign\(false\)/, "alarms must provide automatic campaign succession without scheduling until the cycle finishes");
