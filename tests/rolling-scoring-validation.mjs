...
assert.deepEqual([1,2,3,4].map(n=>roundDefinition(n).type),["CAPITAL_MISSION","PRECISION_MISSION","STREAK_MISSION","CAPITAL_MISSION"]);
assert.equal(decorateRound(state.round).mission.objective,"BONUS POINTS FOR MOST MONEY EARNED");
...
const settlements=settleElapsedRounds(state,startedAt+240001);assert.equal(settlements.length,1);assert.equal(settlements[0].roundType,"CAPITAL_MISSION");assert.equal(settlements[0].categoryWinners.mostMoney[0].agentId,"CODY");assert.equal(settlements[0].categoryWinners.highestReturn[0].agentId,"ATLAS");assert.equal(settlements[0].categoryWinners.streak[0].agentId,"CODY");assert.equal(settlements[0].missionBonusWinners[0].agentId,"CODY");assert.equal(state.scoring.campaignScores.CODY,60);assert.equal(state.scoring.campaignScores.ATLAS,-10);
...
assert.ok(state.scoring.events.every(event=>Number.isInteger(event.pointsDelta)));assert.ok(Object.values(state.scoring.campaignScores).every(Number.isInteger));assert.equal(publicScoring(state).rulesVersion,"rolling-mission-bonus-v2");
...
const precision=stateForRound(2);precision.trades.push(roundTrade(precision,"pc","CODY",200,2,30000),roundTrade(precision,"pa","ATLAS",100,4,40000));const precisionCash=precision.agents.CODY.cashUsd=1000000;precision.agents.ATLAS.cashUsd=1000000;const precisionSettlement=settleElapsedRounds(precision,Date.parse(precision.round.endsAt)+1)[0];assert.equal(precisionSettlement.roundType,"PRECISION_MISSION");assert.equal(precision.scoring.campaignScores.CODY,20);assert.equal(precision.scoring.campaignScores.ATLAS,40);assert.equal(precision.agents.CODY.cashUsd,precisionCash,"points must not alter financial balances");

const streakRound=stateForRound(3);streakRound.trades.push(roundTrade(streakRound,"s1","CODY",100,2,30000),roundTrade(streakRound,"s2","CODY",80,1,50000),roundTrade(streakRound,"s3","ATLAS",90,3,60000));const streakSettlement=settleElapsedRounds(streakRound,Date.parse(streakRound.round.endsAt)+1)[0];assert.equal(streakSettlement.roundType,"STREAK_MISSION");assert.equal(streakSettlement.categoryWinners.streak[0].agentId,"CODY");assert.equal(streakRound.scoring.campaignScores.CODY,60);assert.equal(streakRound.scoring.campaignScores.ATLAS,20);

const tie=stateForRound(2);tie.trades.push(roundTrade(tie,"tc","CODY",100,1,30000),roundTrade(tie,"ta","ATLAS",100,2,50000));const tieSettlement=settleElapsedRounds(tie,Date.parse(tie.round.endsAt)+1)[0];assert.deepEqual(tieSettlement.categoryWinners.mostMoney.map(row=>[row.agentId,row.points]),[["CODY",10],["ATLAS",10]]);assert.ok(tieSettlement.categoryWinners.mostMoney.every(row=>Number.isInteger(row.points)));

const migrated=stateForRound(4);migrated.scoring.rulesVersion="rolling-round-scoring-v1";migrated.scoring.campaignScores={CODY:84,ATLAS:79};const oldEvents=migrated.scoring.events.length;ensureRollingScoring(migrated,Date.parse(migrated.round.startedAt));assert.deepEqual(migrated.scoring.campaignScores,{CODY:84,ATLAS:79});assert.ok(migrated.scoring.events.length>=oldEvents);
console.log(JSON.stringify({rotation:"passed",baseCategories20:"passed",missionBonus20:"passed",maximumCategory40:"passed",capitalMission:"passed",precisionMission:"passed",streakMission:"passed",qualifiedPassPenalty:"passed",breakEvenCap:"passed",wipeoutIdempotency:"passed",streak:"passed",wholeNumberTie:"passed",financialIsolation:"passed",migrationPreservesScores:"passed",wholeNumbers:"passed",settlementIdempotency:"passed"},null,2));
